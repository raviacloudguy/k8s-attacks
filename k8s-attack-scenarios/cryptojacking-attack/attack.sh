...
set -eu
# In this example we will see the impact of Cryptojacking attack

# Launch the pod with writable root container file system in the specific user namespace and use that as an attack entry point to run their crypto worms

echo "\n"
echo "************ Blue Team actions are started  **************"
echo "\n"

echo "Step 1: Vulnerable aapplication container with writable root file system is deployed"
...
echo "\n"
echo "*********** Warning : Application container is compromised - Cryptominer start running ... ***********"
echo "\n"
echo "Processes running inside the application container:"
kubectl exec -n "$NS" -it "$POD" -- sh -c "sleep 5; ps -C sh -C ps -N -o pid,comm"
